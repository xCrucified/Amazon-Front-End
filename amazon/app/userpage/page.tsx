...
const UserPage: React.FC<Props> = () => {
  ...
  return (
    <div className='w-full'>
      ...
      {
        isEditing && (
          <div className='mb-[24px] text-lg'>If you want to change the name associated with your Onyx customer account, you<br/>may do so below. Be sure to click the <b>Save Changes</b> button when you are done.</div>
        )
      }
      <Form {...form}>
        <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
          <FormField
            ...
            render={({field}) => (
              <FormItem className="flex w-full items-center justify-between py-[12px] px-[24px] rounded-md bg-white">
                <FormLabel className="text-right bg-white font-medium text-[16px]">{isEditing ? 'New Name' : 'Name'}</FormLabel>
                <FormControl>
              ...
              <FormItem className="flex w-full items-center justify-between py-[12px] px-[24px] rounded-md bg-white">
                {
                  ...
                  ? <>
                      <div className="flex flex-col items-start">
                        <div className='text-left bg-white font-medium text-[16px] mb-[16px]'>Enter the new email address you would like to associate with your account<br/>below. We will send a One Time Password (OTP) to that address. Name</div>
          ...
          <Button variant="ghost" className={`font-bold text-lg ${isEditing ? 'text-[#E16C60] hover:text-[#E16C60': 'text-black'}`} onClick={handleEditClick}>
